...
'''
    ...
    while total_rate>0 and t<tmax:
        ...
        # @TODO: Should be able to do the edge breaking/creating here, based on the new status:
        #   I. Check if the new status is Q, if so do the rewiring logic here.
        #       Ia. Contact network is represented by the NetworkX graph G.
        ...
        # @TODO: First a quick primer on how events work:
        #   I. Each node generates future transition events, sampled from a PPP with the relevant rate parameter.
        #   II. These events are pushed into a priority queue.
        #   III. Events are pulled one by one from the priority queue, time jumps between events.
        #   So in order to for the edge rewiring to work, whenever the Contact graph G is modified, the priority queue
        #       also needs to be updated.
        #       IV. Remove stale future transitions from the priority queue. (The one that would've involved the quarantined node.)
        #       V. Push the new events generated by the introduction of a new node or new edges to the priority queue.
